...
createContextMenusEvent.pipe(switchMap(() => getConfig())).subscribe((config) => {
...
reloadTabEvent.pipe(
    switchMap(config => from(chrome.tabs.query({
        currentWindow: (config.window_type === 0) ? undefined : true,
        pinned: (config.tab_type === 0) ? undefined : (config.tab_type === 1),
        audible: (config.exclude_audible_tabs) ? false : undefined
    })).pipe(
        mergeMap(tabs => from(tabs)),
        concatMap(tab => (tab.id != null) ? from(chrome.tabs.reload(tab.id, { bypassCache: config.bypass_cache })) : of(undefined))
    ))
).subscribe();
...
chrome.storage.onChanged.addListener(function (changes, areaName) {
    chrome.contextMenus.removeAll(() => { createContextMenusEvent.next(); });
});

chrome.action.onClicked.addListener(function (tab) {
    getConfig().subscribe(config => { reloadTabEvent.next(config); });
});
    ...
    getConfig().subscribe(config => {
        switch (info.menuItemId) {
            case "reload_all_windows":
                reloadTabEvent.next({ window_type: 0, tab_type: 0, bypass_cache: config.bypass_cache, exclude_audible_tabs: config.exclude_audible_tabs });
                return;
            case "reload_this_window":
                reloadTabEvent.next({ window_type: 1, tab_type: 0, bypass_cache: config.bypass_cache, exclude_audible_tabs: config.exclude_audible_tabs });
                return;
            case "reload_pinned_tabs":
                reloadTabEvent.next({ window_type: 0, tab_type: 1, bypass_cache: config.bypass_cache, exclude_audible_tabs: config.exclude_audible_tabs });
                return;
            case "reload_unpinned_tabs":
                reloadTabEvent.next({ window_type: 0, tab_type: 2, bypass_cache: config.bypass_cache, exclude_audible_tabs: config.exclude_audible_tabs });
                return;
            case "bypass_cache":
                void chrome.storage.local.set({ bypass_cache: info.checked });
                return;
            case "exclude_audible_tabs":
                void chrome.storage.local.set({ exclude_audible_tabs: info.checked });
